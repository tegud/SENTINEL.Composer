...
module.exports = function(sessionLog) {
	...
	if(furthestPointReached === 'waitonipg') {
		...
		var positionOfLastWait = _.chain(sessionLog.events).map(function(event) {
			return event.domainEventType === 'booking journey event' ? event.type + '.' + event.event
			 : event.type; 
		}).lastIndexOf('domain_events.waitonipg').value();
